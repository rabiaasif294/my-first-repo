...
def is_winner(board, player):
    ...
    for i in range(3):
        if board[i][0] == board[i][1] == board[i][2] == player:  # Check row
            return True
        if board[0][i] == board[1][i] == board[2][i] == player:  # Check column
            return True
    # Check diagonals
...
def is_full(board):
    ...
    for row in board:
        for cell in row:
            if cell == ' ':  # If there is any empty cell, the board is not full
                return False
    return True

def minimax(board, depth, is_maximizing):
    """Minimax algorithm to evaluate the best board position."""
